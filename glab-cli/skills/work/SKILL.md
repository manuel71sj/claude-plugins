...
5. **신규 이슈 생성** (사용자가 신규 선택 시):
   - 이슈 목적, 배경, 작업 범위를 포함하여 Write 도구로 `/tmp/gl-body.md`를 작성한다.
...
4. **이슈 본문 업데이트** — 기존 description을 조회한 후 작업내용을 추가한다.
   ...
   - Write 도구로 `/tmp/gl-body.md`에 업데이트된 전체 description을 작성한다.
...
2. **완료보고 작성** — 변경사항 요약, 테스트 결과 등을 포함하여 Write 도구로 `/tmp/gl-body.md`를 작성한다.
...
4. **남은 작업 → 후속 이슈 생성** (남은 작업이 있는 경우):
   ...
   - Write 도구로 `/tmp/gl-body.md`에 후속 이슈 description을 작성한다.
