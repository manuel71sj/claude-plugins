...
| **work** | `/glab-cli:work` | Issue 수명주기 워크플로우 (시작→진행→완료 자동 관리) |
...
├── work/
│   └── SKILL.md              # 이슈 수명주기 워크플로우 (시작→진행→완료)
└── ci/
...
/glab-cli:work     # Issue 수명주기 워크플로우 (시작→진행→완료)
```
...
| `작업 시작`, `구현 진행`, `구현 완료`, `work start` | `/glab-cli:work` |
