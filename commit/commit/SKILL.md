...
브랜치가 glab 이슈에 대응하면 꼬리말에 이슈 참조를 추가합니다.
...
# 매칭 패턴: GH-N, #N, N-slug
ISSUE_NUM=$(echo "$BRANCH" | grep -oE '(^|[/_-])(GH-|#)?[0-9]+' | head -1 | grep -oE '[0-9]+')
```

대응 브랜치 패턴 예시:
- `feature/GH-110-console-user-model` → `110`
- `feature/123-add-login` → `123`
- `fix/#45-bug` → `45`
- `110-user-model` → `110`

**2. glab 이슈 존재 확인** (선택적, 실패 시 무시)

```bash
glab issue view "$ISSUE_NUM" 2>&1 | sed 's/\x1b\[[0-9;]*[a-zA-Z]//g' | head -5
```

- 이슈 존재하지 않거나 glab 미인증 시: 트레일러 추가 생략, 경고 후 계속 진행
- 존재 시: 제목 확인 후 트레일러 추가

**3. 꼬리말 추가**

커밋 메시지 본문 뒤 빈 줄 다음에 꼬리말 삽입:

| 타입/상황 | 트레일러 |
|-----------|----------|
| 기본 (feat, refactor, chore 등) | `Refs: #<N>` |
| 버그 수정 완료 (fix + 이슈 종료 의도) | `Closes: #<N>` |
| 부분 수정 (이슈 미종료) | `Refs: #<N>` |
...
다음 경우 이슈 번호 추가를 생략:
- 브랜치명에서 숫자 추출 실패
- `main`, `master`, `develop` 등 보호 브랜치
- `--message`로 메시지 직접 지정 (사용자 입력 우선)
- glab CLI 미설치 또는 GitLab 저장소가 아닌 경우

**6. 사용자 확인**

추출된 이슈 번호는 Step 3 확인 단계 미리보기에 포함되어 사용자가 검토/수정 가능.
